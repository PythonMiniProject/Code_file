# -*- coding: utf-8 -*-
"""
Created on Fri Apr 30 14:59:23 2021
...
# we inintialized values of variables in from entrybox
    ...
    if name =="" or address == "" or phone == "" or email == "" or age == "" or adharcard == "" or password ==""\
        ...
        tmsg.showwarning("INCOMPLETE NUMBER","Your mobile no. must be 10 digits")
        ...
        with open("accounts.txt","a") as f:
            ...
            f'{datevalue.get()}/{monthvalue.get()}/{yearvalue.get()} {age} '
            f'{adharcard} {password} {gender} {account_type} 0.0\n')
        tmsg.showinfo("CONGRATULATIONS","YOUR ACCOUNT HAS SUCCESSFULLY CREATED")
...
def deposit():
    ...
    def finish_deposit():
        ...
        temp = []
        temp = words[num:num+11]
        #print(temp)
        #print("****")
        current_balance = words[num+10]
        updated_balance = current_balance
        updated_balance = float(updated_balance) + float(deposit_amount.get())
        data       = data.replace(current_balance, str(updated_balance))
        f.seek(0)
        f.truncate(0)
        f.write(data)
        f.close()
       ...
       # balance = int(words[num+31]) + deposit_amount.get()
        #print(balance)
    
    deposit_screen = Toplevel(root)
    deposit_screen.geometry("750x600")
    deposit_screen.title("DEPOSIT MONEY")
    Label(deposit_screen,text = " Enter name: ",font = 'Lucida 10 bold',bg = 'green',fg = 'white').place(x = 44,y = 10)
    Label(deposit_screen,text = " Enter amount: ",font = 'Lucida 10 bold',bg = 'green',fg = 'white').place(x = 44,y = 50)
    #current_balance_label = Label(deposit_screen, font=('Calibri',12)).place(x=250,y=180)
    #deposit_notif = Label(deposit_screen,font=('Calibri',12)).place(x=250,y=150)
    Entry(deposit_screen,textvariable = namecheck,font="comicsans 10 bold").place(x=250,y= 10)
    Entry(deposit_screen,textvariable = deposit_amount,font="comicsans 10 bold").place(x=250,y= 50)
    Button(deposit_screen,text = 'OK',bg = 'yellow green',command = finish_deposit).place(x = 250,y = 100)
    #f.close()
...
def withdrawal():
    ...
    def finish_withdrawal():
        ...
        current_balance = words[num+10]
        updated_balance = current_balance
        updated_balance = float(updated_balance) - float(withdrawal_amount.get())
        data       = data.replace(current_balance, str(updated_balance))
        f.seek(0)
        f.truncate(0)
        f.write(data)
    ...
    Label(withdraw_screen,text = " Enter name: ",font = 'Lucida 10 bold',bg = 'green',fg = 'white').place(x = 44,y = 10)
    Label(withdraw_screen,text = " Enter amount: ",font = 'Lucida 10 bold',bg = 'green',fg = 'white').place(x = 44,y = 50)
    Entry(withdraw_screen,textvariable = namecheck,font="comicsans 10 bold").place(x=250,y= 10)
    Entry(withdraw_screen,textvariable = withdrawal_amount,font="comicsans 10 bold").place(x=250,y= 50)
    Button(withdraw_screen,text = 'OK',bg = 'yellow green',command = finish_withdrawal).place(x = 250,y = 100)
    #f.close()
...
img4=ImageTk.PhotoImage(Image.open("img4.png"))
...
#Label(frame2,image = img).place(x = 60, y = 30)
#frame2.pack(padx = 20,fill = 'both')
...
Button(root,text = 'Deposit',bg = 'yellow green',command = deposit).place(x = 350,y = 434)
Button(root,text = 'Withdraw',bg = 'yellow green',command = withdrawal).place(x = 420,y = 434)
